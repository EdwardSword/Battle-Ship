...
createboard('darkblue', 'player')
createboard('darkblue', 'bot')
	...
	if(!gameOver){
		...
		setTimeout(() => {
			...
			if (allBoardBlocks[randomGo].classList.contains('taken') && allBoardBlocks[randomGo].classList.contains('boom') ) {
				...
				infoDisplay.textContent = "We've lost a command post!"
...
function checkSore(user, userHits, UserSS) {
	...
	if(playerSunkShips.length === 5) {
		infoDisplay.textContent = 'GG, you won!'
		gameOver = true
	}
	if(botSunkShips.length === 5) {
		infoDisplay.textContent = 'GG, you lost!'
		gameOver = true
	}
}
